...
Requires a QuantLog checkout (sibling ``quantlogv1`` or ``QUANTLOG_REPO_PATH``); otherwise skips.
    ...
    def test_signal_evaluated_desk_grade_passes_quantlog_validator(self) -> None:
        ...
        if validate_path is None:
            self.skipTest(
                ...
                "quantlogv1 beside quantbuildv1, and ensure quantlog dependencies are installed"
            )
        try:
