...
Vanuit **repo root** (`quantbuildv1`):

```powershell
$qb = "C:\Users\Gebruiker\quantbuildv1"   # pas aan indien nodig
...
1. **QuantAnalytics** (post-run): rapport + `*_KEY_FINDINGS.md` onder `quantanalyticsv1/output_rapport/` — per run al gefilterd op `run_id` als post-run aan stond.
