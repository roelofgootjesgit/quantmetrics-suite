...
| Runtime voor de bot | **Altijd** de interpreter uit `quantbuild/.venv/bin/python` |
...
```text
/opt/quantbuild/quantbuild     ← hoofd-app, hier staat de .venv van de bot
/opt/quantbuild/quantbridge      ← library / OpenAPI-laag (geen tweede bot-venv nodig tenzij je bewust dev-test doet)
/opt/quantbuild/quantlog         ← event spine (validate / replay / CLI); geen eigen productie-venv
...
JSONL-eventdagen staan **niet** in de QuantLog-repo: typisch onder QuantBuild, bv. `quantbuild/data/quantlog_events/<YYYY-MM-DD>/` (via config `quantlog.base_path`).
...
|--------|----------------|------------|----------------|
| **QuantBuild** (bot, systemd) | `quantbuild` | **Ja** — `.venv` | `requirements.txt` van **deze** repo |
| **QuantBridge** | `quantbridge` | **Nee** voor productie-run | QuantBuild’s venv bevat runtime-deps; bridge wordt als **bronpad** ingeladen |
| **QuantLog** | `quantlog` | **Nee** voor productie-run | Zelfde **QuantBuild** `.venv`; optioneel `pip install -e /opt/quantbuild/quantlog` voor `python -m quantlog.cli`. `scripts/quantlog_post_run.py` zet ook `PYTHONPATH` naar `quantlog/src` — dan is editable install niet verplicht voor post-run alleen |

**Waarom geen tweede venv voor alleen bridge op productie?**  
De live service start vanuit QuantBuild; die laadt `quantbridge` via `sys.path` + `QUANTBRIDGE_SRC_PATH`. Wijzigingen in bridge-code zijn dus: **pull in `quantbridge` → restart systemd** — geen aparte bridge-venv nodig, zolang je geen extra Python-packages in bridge toevoegt die niet in QuantBuild’s `requirements.txt` staan.

**Waarom geen QuantLog-eigen `.venv` op productie?**  
Hetzelfde principe: één interpreter (3.10.x) in QuantBuild’s venv. Na pull in `quantlog`: **herstart systemd** (als je runtime QuantLog-code pad gebruikt) en/of opnieuw `pip install -e …/quantlog` in QuantBuild’s venv als je de CLI zo aanroept.
...
De **live bot** blijft §2 volgen: **één** interpreter, `quantbuild/.venv`. Dat wijzigen we niet.

De **QuantLog Ops Console** is een **aparte, read-only web-UI** (Streamlit + `pandas` via `pyproject.toml` optional `ops`). Die draait **niet** in de trading hot path. Een **eigen** virtualenv in dezelfde `quantlog`-repo (bijv. `pip install -e ".[ops]"` in `quantlog/.venv`) is daarom aanbevolen:
...
**systemd** voor de Ops Console moet **`ExecStart=`** laten wijzen naar **`…/quantlog/.venv/bin/python`**, niet naar QuantBuild’s `.venv`. Deploy-stappen en env-vars: QuantLog-repo **`docs/VPS Deploy Handboek — QuantLog Ops Console v0.1.md`**.
...
pip install -e /opt/quantbuild/quantlog   # optioneel: als je quantlog als package in deze venv wilt (CLI); anders alleen PYTHONPATH zoals post_run
deactivate
...
3. Pull **QuantBuild**.  
4. `pip install -r requirements.txt` alleen in **QuantBuild** `.venv` als nodig; optioneel `pip install -e /opt/quantbuild/quantlog`.  
...
Verwacht: pad eindigt op `quantbuild/.venv/bin/python` en versie **3.10.x**.
...
```bash
/opt/quantbuild/quantbuild/.venv/bin/pip install -r requirements.txt
```
...
```text
/opt/quantbuild/quantbuild/.venv/bin/python -m src.quantbuild.app ...
```
...
tail -n 80 /opt/quantbuild/quantbuild/logs/runtime_ctrader_demo.log
```
...
- [ ] QuantLog gewijzigd en je gebruikt `pip install -e`? → opnieuw `pip install -e /opt/quantbuild/quantlog` in **QuantBuild** `.venv`  
- [ ] `python --version` in `.venv` = 3.10.x  
- [ ] `QUANTBRIDGE_SRC_PATH` wijst naar `.../quantbridge/src`  
- [ ] `systemctl restart` uitgevoerd  
- [ ] Log + Telegram (indien aan) gecontroleerd  
- [ ] QuantLog Ops Console gewijzigd? → `pip install -e ".[ops]"` in **`quantlog/.venv`** (niet in QuantBuild’s venv) + `systemctl restart` van de console-unit (zie §4.1)
...
*Last aligned: 2026-04-19 — Python 3.10 contract, QuantBridge + QuantLog + QuantBuild pull order, single QuantBuild venv; QuantLog Ops Console (§4.1) optional eigen venv in `quantlog`.*
