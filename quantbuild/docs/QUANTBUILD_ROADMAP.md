...
**Doel:** backlog **alleen voor deze repo** (`quantbuild`), afgestemd op het gedeelde platform-document in **QuantLog**: `docs/PLATFORM_ROADMAP.md` (correlatie, canonieke reasons, lifecycle, nightly rapportage).  
...
| P4.3 | **Default `--quantlog-repo-path`:** VPS `/opt/quantbuild/quantlog`, env `QUANTLOG_REPO_PATH`, anders sibling `quantlog` | ✅ `scripts/quantlog_post_run.py` |
