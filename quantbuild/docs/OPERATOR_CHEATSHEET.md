...
Pas `/opt/quantbuild/…` aan naar jouw pad (bijv. `/root/dev/quant/quantbuild`):

```bash
export QB=/root/dev/quant/quantbuild
cd "$QB" && git pull --ff-only
cd /root/dev/quant/quantbridge && git pull --ff-only
cd /root/dev/quant/quantlog && git pull --ff-only
...
pip install -e /root/dev/quant/quantlog   # nodig voor quantlog_post_run / CLI
```
...
**A — systemd** (units moeten naar jouw `WorkingDirectory` / `ExecStart`-pad wijzen; standaard in repo = `/opt/quantbuild/quantbuild`):
...
QUANTBUILD_ROOT=/root/dev/quant/quantbuild ./scripts/vps/start_weekrun.sh
```
...
/root/dev/quant/quantbuild/.venv/bin/python quantmetrics.py build -c configs/demo_strict_ctrader.yaml
...
**Handmatig vanuit `quantbuild` met secrets in `orchestrator/.env`:** `quantmetrics.py` laadt die `.env`; een kale `python -m src.quantbuild.app …` **niet**. Gebruik:  
...
**Timer** (eenmalig): standaard unit-bestanden gebruiken `/opt/quantbuild/quantbuild` — pas **`deploy/systemd/quantbuild-quantlog-report.service`** naar jouw pad aan **of** kopieer handmatig naar `/etc/systemd/system/` met juiste paden, daarna `install_quantlog_nightly_timer.sh` (of handmatig `systemctl enable --now` op timer). Zie §9.

**Handmatig nu** (zonder systemd, jouw pad):

```bash
export QUANTBUILD_ROOT=/root/dev/quant/quantbuild
export QUANTBUILD_POST_RUN_CONFIG=configs/strict_prod_v2.yaml
export QUANTLOG_REPO_PATH=/root/dev/quant/quantlog
...
**QuantLog-repo-pad:** zet **`QUANTLOG_REPO_PATH`** (of alias **`QUANTLOG_ROOT`**) naar je clone, bv. `/root/dev/quant/quantlog`. Zonder geldige clone faalt post-run vóór validate.
...
- **File log** (cTrader-demo unit, huidige VPS): `tail -f /opt/quantbuild/quantbuild_e1_v1/logs/runtime_ctrader_demo.log` — elders vaak `…/quantbuild/logs/runtime_ctrader_demo.log`
...
# cd /opt/quantbuild/quantbuild && tail -n 80 logs/runtime_ctrader_demo.log
```
...
Zie `docs/VPS_MULTI_MODULE_DEPLOYMENT.md`: na pull eventueel `pip install -r requirements.txt` en/of `pip install -e /opt/quantbuild/quantlog` in **QuantBuild** `.venv` vóór restart.

**Eerste keer op deze VPS:** als `ls /opt/quantbuild/quantlog` faalt → in `/opt/quantbuild` uitvoeren: `git clone <jouw-QuantLog-repo-URL> quantlog`, daarna `pip install -e /opt/quantbuild/quantlog` in de QuantBuild-venv. Zonder deze clone werkt `python -m quantlog.cli` / `scripts/quantlog_post_run.py` niet op de server (events staan wél in `data/quantlog_events/`).
...
# repo-standaard: tail -f /opt/quantbuild/quantbuild/logs/runtime_ctrader_demo.log
...
Vereist: `quantlog` gecloned + `pip install -e /opt/quantbuild/quantlog` in QuantBuild `.venv`.
...
tail -n 80 /opt/quantbuild/quantbuild/logs/quantlog_nightly.log
```
...
```bash
cd /opt/quantbuild/quantbuild && source .venv/bin/activate
