...
**Productnamen:** **QuantOS** — Orchestrator · **QuantBuild** — Signal Engine · **QuantBridge** — Execution Engine · **QuantLog** — Observability Layer. **Clone-mappen** blijven o.a. `quantmetrics_os`, `quantbuild` (zie `quantmetrics_os/scripts/clone_quant_suite.sh`).
...
|---------------------|-----|
| **quantbuild** | Bot, orchestratie, productie-config, eventpad `data/quantlog_events/`, CI, VPS-systemd. **Hier documenteer je welke combinatie van andere repos je hebt getest.** |
| **quantbridge** | Broker/OpenAPI-laag; wordt op de VPS als **bronpad** geladen (`QUANTBRIDGE_SRC_PATH`). Geen tweede productie-venv tenzij bewust anders afgesproken. |
| **quantlog** | Event-spine: schema, validate/replay/CLI; contract met wat QuantBuild emitteert. |
...
Paden op de VPS zijn vastgelegd in `VPS_MULTI_MODULE_DEPLOYMENT.md` (onder `/opt/quantbuild/…`). Lokaal op Windows mag je dezelfde mapnamen naast elkaar zetten, bijv. `C:\Users\…\quantbuild` naast siblings `quantbridge`, `quantlog`, `quantmetrics_os`.
...
**Standaardpaden:** beide scripts nemen de **oudermap van `quantbuild`** als root en zoeken daar siblings: `quantbuild`, `quantbridge`, `quantlog`, `quantmetrics_os`. Ontbrekende mappen of geen `.git` → **waarschuwing en verder** (andere repos worden wél gepusht).

**Windows (vanuit quantbuild):**
...
Andere remote of andere bovenliggende map (als je repos niet direct naast `quantbuild` staan):
...
Als **alleen** QuantBuild feature-werk doet zonder contractwijziging: normale feature branch in quantbuild volstaat.
...
1. `git pull --ff-only` in **quantbridge** (juiste branch).  
2. Idem **quantlog**.  
3. Idem **quantbuild**.  
4. Alleen in **QuantBuild** `.venv`: `pip install -r requirements.txt` indien nodig; optioneel `pip install -e …/quantlog` als je die workflow gebruikt.  
