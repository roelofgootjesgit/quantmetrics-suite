...
def discover_quantanalytics_output_rapport(quantbuild_root: Path | None = None) -> Path | None:
    """Direct sibling ``../quantanalytics/output_rapport`` (one parent hop only).

    Used so backtest-triggered analytics writes reports into the analytics clone that
    sits **next to** ``quantbuild`` under the same parent folder — not by walking up
...
def invoke_quantanalytics_after_quantlog(
    ...
    """Run QuantAnalytics CLI on the QuantLog tree so reports land under ``quantanalytics/output_rapport``.
    ...
    if chk.returncode != 0:
        logger.warning(
            ...
            "(install in this env: pip install -e /path/to/quantanalytics). stderr=%s",
