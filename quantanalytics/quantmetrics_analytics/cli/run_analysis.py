...
def discover_quantbuild_quantlog_dir() -> Path | None:
    ...
    3. Multi-root workspace: sibling ``quantbuild/data/quantlog_events`` (shallow scan).
...
def _repo_root() -> Path:
    ...
    # Editable checkout: .../quantanalytics/quantmetrics_analytics/cli/run_analysis.py
...
def _output_rapport_dir() -> Path:
    ...
    ``quantanalytics`` checkout from :func:`os.getcwd` (current or parent dirs,
    or sibling ``quantanalytics/`` when using a multi-root workspace), then
    ...
    cwd = Path.cwd().resolve()
    # Inside clone: .../quantanalytics (or renamed folder with same layout)
    for d in [cwd, *cwd.parents]:
        if _is_quantanalytics_clone_root(d):
            return d / "output_rapport"

    # Multi-root workspace: a parent folder contains sibling ``quantanalytics/``.
