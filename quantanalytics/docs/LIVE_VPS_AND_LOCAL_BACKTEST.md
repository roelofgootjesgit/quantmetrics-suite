...
```powershell
cd C:\Users\Gebruiker\quantanalytics
pip install -e .
...
```bash
cd /pad/naar/quantanalytics
/path/naar/quantbuild/.venv/bin/pip install -e .
/path/naar/quantbuild/.venv/bin/python -m quantmetrics_analytics.cli.run_analysis \
...
| Onderwerp | Doc |
|-----------|-----|
| VPS directory layout, één venv | `quantbuild/docs/VPS_MULTI_MODULE_DEPLOYMENT.md` |
| QuantLog op VPS (geen tweede venv) | `quantlog/docs/VPS_SYNC.md` |
| Orchestrator env + paden | `quantmetrics_os/orchestrator/config.vps.example.env` |
| Dagelijkse JSON / mentor-workflow | `quantbuild/docs/VPS_JSON_DAGELIJKSE_ANALYSE_WORKFLOW.md` |

---

## Deel B — Backtest lokaal draaien (QuantBuild)

Analytics op **live JSONL** en backtest zijn hetzelfde **event-schema**, zolang QuantLog in de backtest-config **aan** staat: de engine kan naar o.a. `data/quantlog_events/runs/<run_id>.jsonl` schrijven (zie implementatie in `quantbuild/src/quantbuild/backtest/engine.py` en je YAML-sectie `quantlog`).

### B.1 Voorbereiding (eenmalig)

```powershell
cd C:\Users\Gebruiker\quantbuild
python -m venv .venv
...
(Zie `quantbuild/docs/OPERATOR_CHEATSHEET.md` §0.6b voor edge-unlock varianten.)
...
cd C:\Users\Gebruiker\quantanalytics
pip install -e .
python -m quantmetrics_analytics.cli.run_analysis --jsonl "C:\Users\Gebruiker\quantbuild\data\quantlog_events\runs\<run_id>.jsonl" --reports all
```

Het rapport staat daarna onder **`quantanalytics\output_rapport\`** (zie stderr voor het exacte pad). Alleen naar de console: voeg **`--stdout`** toe.
...
| Backtest | `quantbuild`: `python -m src.quantbuild.app backtest --config …` |
