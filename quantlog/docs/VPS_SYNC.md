...
|------------------------|--------|
| `docs/VPS_MULTI_MODULE_DEPLOYMENT.md` | **Contract:** Python **3.10.x**, één `.venv` alleen in `quantbuild`, geen tweede bot-venv voor Bridge, `QUANTBRIDGE_SRC_PATH`, pull-volgorde, systemd-pad, checklist |
...
- Clone onder `/opt/quantbuild/quantlog` (of de naam van jouw GitHub-repo).
...
Zo blijft er **één** Python-wereld: `quantbuild/.venv`, versie **3.10.x**, zoals het contract voorschrijft.
...
```text
/opt/quantbuild/quantbuild     ← enige bot-venv: .venv hier
/opt/quantbuild/quantbridge      ← bron, geen productie-venv
/opt/quantbuild/quantlog         ← bron, geen productie-venv
/etc/quantbuild/quantbuild.env       ← o.a. QUANTBRIDGE_SRC_PATH
```

Event-JSONL staat **niet** in de QuantLog-repo, maar volgt QuantBuild-config (`quantlog.base_path`, typisch `data/quantlog_events/<datum>/` onder `quantbuild`).
...
```bash
cd /opt/quantbuild
git clone <JOUW_QUANTLOG_REPO_URL> quantlog
```
...
pip install -e /opt/quantbuild/quantlog
deactivate
```

Als je **geen** `pip install -e` wilt: voor CLI-scripts kun je hetzelfde doen als `scripts/quantlog_post_run.py` in QuantBuild — `PYTHONPATH=/opt/quantbuild/quantlog/src` zetten bij aanroep met `quantbuild/.venv/bin/python`.
...
Werkt `python -m quantlog.cli` niet: `pip install -e /opt/quantbuild/quantlog` opnieuw uitvoeren in deze venv, of tijdelijk:

```bash
PYTHONPATH=/opt/quantbuild/quantlog/src python -m quantlog.cli validate-events --path …
```
...
python scripts/quantlog_post_run.py \
  ...
  --quantlog-repo-path /opt/quantbuild/quantlog \
  --date 2026-04-01
```
...
- [ ] QuantLog-code gewijzigd en je gebruikt `pip install -e`? → opnieuw `pip install -e /opt/quantbuild/quantlog` in **QuantBuild** `.venv`  
- [ ] Geen `python`/`pip` voor de stack **buiten** `quantbuild/.venv` (zie `VPS_MULTI_MODULE_DEPLOYMENT.md` §6)
