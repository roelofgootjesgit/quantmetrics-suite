...
Op de VPS geldt in QuantBuild **`docs/VPS_MULTI_MODULE_DEPLOYMENT.md`** (§2–4): **één bot-virtualenv** onder `quantbuild/.venv` voor live run, QuantBridge-laden en (optioneel) `pip install -e` voor QuantLog CLI — bewust geen tweede “runtime”-venv voor spine-code.

De **Ops Console** is daar een **uitzondering** (staat ook in QuantBuild **§4.1**):

| Keuze | Motief |
|--------|--------|
| **Eigen `.venv` in `quantlog`** | Streamlit en gerelateerde UI-deps (`[project.optional-dependencies] ops`) staan **niet** in QuantBuild `requirements.txt`. Die in de bot-venv mixen vergroot kans op versie-conflict en maakt elke `pip install -r requirements.txt` onvoorspelbaarder. |
...
```bash
/opt/quantbuild/data/quantlog_events
```
...
```bash
ls /opt/quantbuild/data/quantlog_events
```
...
```bash
cd /opt
git clone <jouw-repo-url> quantlog
cd quantlog
```
...
**Repo-root:** kan `/opt/quantlog` zijn of bijv. `/root/quant_suite/quantlog` — het gaat om de map waar `pyproject.toml` en `quantlog_ops/` na `git pull` staan. Hieronder: `QUANTLOG_REPO` vervangen door jouw pad.

```bash
cd /path/to/quantlog    # jouw clone
python3 -m venv .venv
...
**Herinnering:** systemd voor deze app gebruikt **`…/quantlog/.venv/bin/python`**, niet `quantbuild/.venv` — zie §4.1 in QuantBuild `VPS_MULTI_MODULE_DEPLOYMENT.md`.
...
```bash
QUANTLOG_OPS_EVENTS_ROOT=/opt/quantbuild/data/quantlog_events
...
```bash
export QUANTLOG_OPS_EVENTS_ROOT=/opt/quantbuild/data/quantlog_events
...
Type=simple
User=root
WorkingDirectory=/opt/quantlog
Environment=QUANTLOG_OPS_EVENTS_ROOT=/opt/quantbuild/data/quantlog_events
...
ExecStart=/opt/quantlog/.venv/bin/python -m streamlit run quantlog_ops/app.py --server.port 8501 --server.address 127.0.0.1
Restart=always
RestartSec=5
...
```bash
ls -lah /opt/quantbuild/data/quantlog_events
```
